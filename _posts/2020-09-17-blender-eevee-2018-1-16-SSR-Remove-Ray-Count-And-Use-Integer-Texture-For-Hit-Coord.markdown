...
## 作用
优化SSR效果，这里的效果觉得比之前的要好,去掉了 多个 ray count ，还有就是 把pdf信息用 rt 保存起来
...
## 准备
可以先看[这里](http://shaderstore.cn/2021/03/16/blender-eevee-2017-7-30-SSR-Rewrote-The-Raytracing-Algorithm/)
...
```
>
- EEVEE_reflection_compute 在主渲染流程会进行调用，主要是进行SSR效果渲染的
<br><br>
- 这里的主要优化就是 在渲染 ssr_raytrace Pass 的时候，先进行绑定 stl->g_data->ssr_pdf_output RT, 出来输出 pdf
<br><br>
- 这个 ssr_pdf_output RT 会传入到 ssr_resolve Pass 中进行使用
...
```
>
- 主要就是 do_planar_ssr 和 do_ssr 函数里面输出 hitData 和 pdfData 到RT 中
...
```
>
- 主要是留意 get_ssr_sample 函数，采样hitBuffer 和 pdfBuffer 后，数据怎么使用